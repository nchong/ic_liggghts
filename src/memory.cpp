...
char **Memory::create_2d_char_array(int n1, int n2, const char *name)
...
void Memory::destroy_2d_char_array(char **array)
...
/* ----------------------------------------------------------------------
   grow or shrink 1st dim of a 2d char array
   last dim must stay the same
   if either dim is 0, return NULL
------------------------------------------------------------------------- */

char **Memory::grow_2d_char_array(char **array, int n1, int n2, const char *name)

{
  if (n1 == 0 || n2 == 0) {
    destroy_2d_char_array(array);
    return NULL;
  }

  if (array == NULL) return create_2d_char_array(n1,n2,name);

  char *data = (char *) srealloc(array[0],n1*n2*sizeof(char),name);
  array = (char **) srealloc(array,n1*sizeof(char *),name);
...
/* ----------------------------------------------------------------------
   grow or shrink 1st dim of a 3d int array
   last 2 dims must stay the same
   if any dim is 0, return NULL
------------------------------------------------------------------------- */

int ***Memory::grow_3d_int_array(int ***array,int n1, int n2, int n3, const char *name)
{
  int i,j;

  if (n1 == 0 || n2 == 0 || n3 == 0) {
    destroy_3d_int_array(array);
    return NULL;
  }

  if (array == NULL) return create_3d_int_array(n1,n2,n3,name);

  int *data = (int *) srealloc(array[0][0],n1*n2*n3*sizeof(int),name);
  int **plane = (int **) srealloc(array[0],n1*n2*sizeof(int *),name);
  array = (int ***) srealloc(array,n1*sizeof(int **),name);
