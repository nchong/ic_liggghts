...
FixPourLegacy::FixPourLegacy(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  if(comm->me==0)error->warning("You are using a legacy command that provides downward compatibility with LAMMPS.\nA new LIGGGHTS command is available that has superceeded this command.");
...
double FixPourLegacy::max_rad(int type)
{
...
void FixPourLegacy::pre_exchange()
{
  int i;
...
int FixPourLegacy::overlap(int i)
{
...
void FixPourLegacy::xyz_random(double h, double *coord)
{
  if (domain->dimension == 3) {
    if (region_style == 1) {
