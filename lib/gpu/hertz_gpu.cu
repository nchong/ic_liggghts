  ...
  double **host_torque, double **host_force)
{
  ...
  //test if flattening is required
  double *h_x2 = (double *)malloc(SIZE_2D);
  memcpy(h_x2, host_x[0], SIZE_2D);
  for (int i=0; i< nall; i++) {
    for (int j=0; j<3; j++) {
      if (h_x[(i*3)+j] != h_x2[(i*3)+j]) {
        printf("error: memcpy for flattening failed.");
        exit(1);
      }
    }
  }
  ...
  step++;
  if (step==1000) {
  //emit access_pattern generated by cell decomposition
  unsigned int *h_idx;
  int *h_natom;
  h_idx = (unsigned int *)malloc(ncell*blockSize*sizeof(unsigned int)); assert(h_idx);
  h_natom = (int *)malloc(ncell*sizeof(int)); assert(h_natom);
  cudaMemcpy(h_idx, cell_list_gpu.idx, ncell*blockSize*sizeof(unsigned int), cudaMemcpyDeviceToHost);
  cudaMemcpy(h_natom, cell_list_gpu.natom, ncell*sizeof(int), cudaMemcpyDeviceToHost);
  //block pattern and thread pattern
  int *access_pattern = (int *)malloc(nall*sizeof(int)); assert(access_pattern);
  int *thread_pattern = (int *)malloc(nall*sizeof(int)); assert(thread_pattern);
  for (int i=0; i<nall; i++) {
    access_pattern[i] = -1;
    thread_pattern[i] = -1;
  }
  for (int i=0; i<ncell; i++) {
    for (int j=0; j<h_natom[i]; j++) {
      int idx = (i * blockSize) + j;
      access_pattern[h_idx[idx]] = i;
      thread_pattern[h_idx[idx]] = j;
    }
  }
  printf("ACCESS PATTERN %d\n", nall);
  for (int i=0; i<nall; i++) {
    printf("> %d, %d, %d\n", i, access_pattern[i], thread_pattern[i]);
  }
  }
